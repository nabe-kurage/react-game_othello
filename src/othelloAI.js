import { COLUMN, squareNum } from "./constData.js";
...
export class GreedyPlayer {
     ...
     * @param {Object} diskSet - see defaultDiskSet for how this is stored
            ...
            directions.forEach((delta_xy) => {
                ...
                if (
                    ...
                    while (
                        ...
                        if (
                            !this.isOpponentColor(
                                boardGrid[newRow][newCol],
                                playerColor
                            )
                        ) {
                            if (XYtoCaptureCounts[xyInt] === undefined) {
                                XYtoCaptureCounts[xyInt] =
                                    potentialCaptureCount;
                            } else {
                                XYtoCaptureCounts[xyInt] +=
                                    potentialCaptureCount;
                            }
    ...
    getSurroundingSquares(diskXYs) {
        ...
        diskXYs.forEach((xy) => {
            let x = xy[0];
            let y = xy[1];
            let maxCol = squareNum.column - 1;
            let maxRow = squareNum.row - 1;
